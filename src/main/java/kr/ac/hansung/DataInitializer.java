...
public class DataInitializer implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        if (!userRepository.existsByEmail("user@hansung.ac.kr")) {
            ...
            normalUser.setEmail("user@hansung.ac.kr");
            normalUser.setPassword(passwordEncoder.encode("user1234"));
            normalUser.getRoles().add(userRole); // ROLE_USER만 추가 (ROLE_ADMIN은 넣지 않음)
            userRepository.save(normalUser);
            log.info("테스트용 일반 계정 생성: user@hansung.ac.kr / user1234");
        }
